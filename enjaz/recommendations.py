...
def get_student_recommendation_by_percentage(percentage, student_name):
    ...
    recommendations = {
        ...
        "يحتاج إلى تطوير": f'{emoji} **يحتاج إلى تطوير ({percentage:.1f}%)**\n\nنسبة إنجاز الطالب للتقييمات الأسبوعية على نظام قطر للتعليم تحتاج إلى تحسين. نأمل من سيادتكم متابعة الطالب وتشجيعه على إنجاز التقييمات بانتظام.',
        
        "لا يستفيد من النظام": f'{emoji} **لا يستفيد من النظام ({percentage:.1f}%)**\n\nالطالب لم ينجز أي تقييم أسبوعي على نظام قطر للتعليم. نرجو من سيادتكم متابعة الطالب بشكل يومي والتواصل مع المدرسة لوضع خطة تحسين مناسبة.'
    }
...
def get_recommendation_for_band(band, student_name=None, level='student'):
    ...
    if band in BAND_LABELS:
        ...
        if band == "البلاتينية":
            ...
            percentage = 50.0
        elif band == "يحتاج إلى تطوير":
            percentage = 20.0
        elif band == "لا يستفيد من النظام":
            percentage = 0.0
        else:
            percentage = None
